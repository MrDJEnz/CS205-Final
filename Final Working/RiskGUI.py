...
class GUI():
    ...
    # Player interface text updates, values define formatting
    def display_hud(self, troopCount, interfaceText, turn, textPosition):
        ...
        textSurface, textBox = self.textArea(turn.players[turn.turnCount - 1].name, smallText, turn.players[turn.turnCount - 1].color)
        ...
        textSurface, textBox = self.textArea("Available number of troops to deploy : " + str(turn.players[turn.turnCount - 1].num_troops), smallText)
        textPosition = (textPosition[0], textPosition[1] - margin - margin)
        textBox.topleft = textPosition
        interfaceText.append([textSurface, textBox])
